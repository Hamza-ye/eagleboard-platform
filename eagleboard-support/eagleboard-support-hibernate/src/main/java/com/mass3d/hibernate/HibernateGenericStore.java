    ...
    implements GenericStore<T>
{
    ...
    protected  TypedQuery<T> getTypedQuery( CriteriaBuilder builder, JpaQueryParameters<T> parameters )
    {
        ...
        {
            List<Predicate> predicates = predicateProviders.stream().map( t -> t.apply( root ) ).collect( Collectors
                .toList() );
        ...
        {
            List<Order> orders = orderProviders.stream().map( o -> o.apply( root ) ).collect( Collectors
                .toList() );
    ...
    protected  Long getCount( CriteriaBuilder builder, JpaQueryParameters<T> parameters )
    {
        ...
        if ( !countExpressions.isEmpty() )
        {
            ...
            {
                query.multiselect( countExpressions.stream().map( c -> c.apply( root ) ).collect( Collectors
                    .toList() ) );
            }
            else
            {
