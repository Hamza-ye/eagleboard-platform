...
import com.mass3d.datavalue.DataValueStore;
import com.mass3d.datavalue.DeflatedDataValue;
...
@Slf4j
@Repository( "com.mass3d.datavalue.DataValueStore" )
    ...
    implements DataValueStore
{
    ...
    public void deleteDataValues( TodoTask todoTask )
    {
        ...
        getSession().createQuery( hql ).
            setParameter( "source", todoTask ).executeUpdate();
    }
...
//            .setParameter( "categoryOptionCombo", categoryOptionCombo ) );
    ...
    public List<DataValue> getDataValues( DataExportParams params )
    {
        ...
        Set<TodoTask> todoTasks = params.getAllTodoTasks();
...
//        }
        ...
        if ( !params.isIncludeChildrenForOrganisationUnits() && !todoTasks.isEmpty() )
        {
            query.setParameterList( "orgUnits", getIdentifiers( todoTasks ) );
        }
...
//            List<Long> deIdList = queryDeos.stream().map( de -> de.getDataElement().getId() ).collect( Collectors
//                .toList() );
...
//        else
        ...
        else if ( params.hasPeriodTypes() || params.hasStartEndDate() || params.hasIncludedDate() )
        {
            ...
            if ( params.hasPeriodTypes() )
            {
                ...
                String periodTypeIdList = getCommaDelimitedString( params.getPeriodTypes().stream().map( o -> o.getId() ).collect( Collectors
                    .toList() ) );
        ...
        {
            List<TodoTask> todoTasks = new ArrayList<>( params.getAllTodoTasks() );
            List<Long> orgUnitIdList = todoTasks.stream().map(  TodoTask::getId ).collect( Collectors
                .toList() );
//            List<String> orgUnitPathList = todoTasks.stream().map(  OrganisationUnit::getPath ).collect( Collectors
//                .toList() );

            sql += " join todotask ou on ou.todotaskid = dv.sourceid";
...
//                + " on ou.path like " + statementBuilder.concatenate( "opath.path", "'%'");
        ...
        else if ( params.hasTodoTasks() )
        {
            String orgUnitIdList = getCommaDelimitedString( getIdentifiers( params.getTodoTasks() ) );
...
//        }
        ...
        while ( rowSet.next() )
        {
            ...
            result.add( new DeflatedDataValue( dataElementId, periodId,
                organisationUnitId,value, storedBy, created, lastUpdated, comment, followup, deleted ) );
        }
