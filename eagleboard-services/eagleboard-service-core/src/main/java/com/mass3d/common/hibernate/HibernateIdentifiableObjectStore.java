    ...
    extends HibernateGenericStore<T> implements GenericDimensionalObjectStore<T>, InternalHibernateGenericStore<T>
{
    ...
    public List<T> getAllLikeName( Set<String> nameWords, int first, int max )
    {
        ...
        param.addPredicate( root -> builder.and( conjunction.stream().map( p -> p.apply( root ) ).collect( Collectors
    ...
    public List<T> getByUidNoAcl( Collection<String> uids )
    {
        ...
        List<List<String>> partitions = Lists
            .partition( Lists.newArrayList( uids ), OBJECT_FETCH_SIZE );
    ...
    @Override
    public  List<Function<Root<T>, Predicate>> getDataSharingPredicates( CriteriaBuilder builder, String access )
    {
    ...
    {
        return false; //Dashboard.class.isAssignableFrom( clazz );
    }
