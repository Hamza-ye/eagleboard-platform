...
import com.mass3d.outboundmessage.*;
    ...
    implements MessageSender
{
        ...
        boolean forceSend )
    {
        ...
        toSendList = users.stream().filter( u -> forceSend || isQualifiedReceiver( u ) ).collect( Collectors
            .toSet() );
    ...
    {
        return sendMessage( subject, text, StringUtils.isBlank( recipient ) ? new HashSet<>() : Sets
            .newHashSet( recipient )  );
    }
    ...
    private void sliceBatchMessages( OutboundMessageBatch batch )
    {
        ...
        messages = batch.getMessages().stream().flatMap( m -> sliceMessageRecipients( m ).stream() ).collect( Collectors
            .toList() );
    ...
    {
        Set<GatewayResponse> okCodes = Sets
            .newHashSet( GatewayResponse.RESULT_CODE_0, GatewayResponse.RESULT_CODE_200,
